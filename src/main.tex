...
\section{Стандарты для обмена ключами}\label{sec:IETF}
...
обозначены три варианта алгоритма, два из которых используют такие подписи.
...
центр сертификации.
Таким образом, при компроментации какого-либо ключа для определённой атакующий не может получить
...
Устаревшие стандарты IETF во имя обратной совместимости довольно долго поддерживаются даже после выхода следующих версий.
...
TLS 1.3 представил достаточно много новых требований, которые делают невозможным использование различных оптимизаций из
предыдущих версий протокола: больше нельзя повторно использовать публичные ключи, допустимые группы, используемые
в протоколе, стали длиннее и т.д.
...
В рамках данной главы была разработана библиотека для арифметики в конечных полях на языке Python.
В библиотеке было реализовано несколько вариаций алгоритмов возведения в степень по модулю.
С использованием этой библиотеки были реализованы различные модификации алгоритмов Диффи-Хеллмана и
...
Ключевая для данной задачи особенность языка Python - это естественная работа с большими числами.
